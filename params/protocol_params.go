...
const (
	...
	BTCCheckpointGas                   uint64 = 5500    // the cost of BTC checkpoint confirmations lookup (DB-backed on cache miss)
	BTCCheckpointLastIndexGas          uint64 = 5500    // the cost of BTC last-checkpoint-index lookup (DB-backed on cache miss)
	// Batched BTC checkpoint lookups (amortize overhead across items).
	// Priced as base+per-item to amortize overhead across batch items.
	BTCCheckpointBatchBaseGas    uint64 = 4500
	BTCCheckpointBatchPerItemGas uint64 = 1000
